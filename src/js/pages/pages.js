...
import { 
  configurarCartButton, 
  verificarUsuarioLogueado, 
  mostrarAlertIniciarSesion, 
  agregarProductoAlCarritoPersistente as agregarProductoAlCarrito, 
  mostrarModalCarrito 
...
customElements.define(
  ...
  class extends HTMLElement {
    ...
    aplicarFiltro(filterId, value) {
      const newFilter = `${filterId}=${value.id}`;
      if (!this.appliedFilters.includes(newFilter)) {
        this.appliedFilters.push(newFilter);
        this.mostrarChipFiltro(filterId, value.id, value.name);
      }
      this.offset = 0;
      this.hasMoreProducts = true;
    ...
    mostrarChipFiltro(filterId, valueId, valueName) {
    ...
    mostrarChipsFiltros() {
      ...
      this.appliedFilters.forEach(filter => {
        ...
        this.mostrarChipFiltro(filterId, valueId, filter.name);
      });
    }
    ...
    escribirListadoProductos(listado) {
      ...
      productCount.innerText = `${listado.paging.total} productos`;
      ...
      fetch(url)
        ...
        .then((data) => {
          ...
          this.filters = data.available_filters
            .filter(filter => filter.values.length > 1) // Filtra los filtros que tienen más de una opción
            .map(filter => {
              return {
                text: filter.name,
                handler: () => {
    ...
    abrirFiltroValuesActionSheet(filter) {
      ...
      actionSheet.buttons = filter.values.map(value => {
        return {
          text: value.name,
          handler: () => {
            this.aplicarFiltro(filter.id, { id: value.id, name: value.name });
          }
        };
      }).concat({
        text: "Cancelar",
  ...
  class extends HTMLElement {
    connectedCallback() {
      ...
      if (this instanceof HTMLElement) {
        ...
        // opciones de usuario en la página de ampliación 
    ...
    escribirAmpliacionProducto(producto) {
      ...
      this.querySelector("#product-brand").innerText = `${producto.attributes.find(attr => attr.id === 'BRAND').value_name}`;
    ...
    configurarBotonAgregarCarrito() {
      ...
      botonAgregarCarrito.addEventListener("click", () => {
        if (verificarUsuarioLogueado()) {
          ...
          mostrarModalCarrito(); // Mostrar el modal del carrito después de agregar el producto
        } else {
          mostrarAlertIniciarSesion();
        }
      });
